module pipelined_machine(clk, reset);
    ...
    //DE/MW
    wire RegWrite_in;
    register #(1) RegWrite_reg(RegWrite, RegWrite_in, clk,  enable, reset|stall);

    wire MemRead_in;
    register #(1) MemRead_reg(MemRead, MemRead_in, clk,  enable, reset|stall);

    wire MemWrite_in;
    register #(1) MemWrite_reg(MemWrite, MemWrite_in, clk,  enable, reset|stall);

    wire MemToReg_in;
    register #(1) MemToReg_reg(MemToReg, MemToReg_in, clk,  enable, reset|stall);

    wire [31:0] alu_out_data_in;
    register #(32) ALU_reg(alu_out_data, alu_out_data_in, clk,  enable, reset|stall);

    wire [31:0] WriteData_in, WriteData;
    register #(32) WriteData_reg(WriteData, WriteData_in, clk,  enable, reset|stall);

    wire [4:0] wr_regnum_in;
    register #(5) wr_regnum_reg(wr_regnum, wr_regnum_in, clk,  enable, reset|stall);
    /////////////////////////////////////////////////////////////////////////////////////////////

    /////////////////////////////////////////////////////////////////////////////////////////////
    //Forwarding Unit & Muxes
    wire   ForwardA, ForwardB;
    assign ForwardA = ((RegWrite == 1) & (wr_regnum == rs) & (wr_regnum != 0));
    assign ForwardB = ((RegWrite == 1) & (wr_regnum == rt) & (wr_regnum != 0));
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////
    //Hazard Unit
    assign stall = (MemRead == 1) & ((wr_regnum == rs) | (wr_regnum == rt)) & (wr_regnum != 0);
    //flush
