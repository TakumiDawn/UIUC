...
#         lw		$s0, flag_puzzle
#     		beq		$s0, 0, go1		# if flag_puzzle is 1, go to solving puzzle
     		jal 	solve_puzzle
# go1:
...
y_smaller_loop:
lw		$s0, flag_puzzle
beq		$s0, 1, go2		# if flag_puzzle is 1, skip request
la		$t8, puzzle_data			# request puzzle
sw      $t8, REQUEST_PUZZLE
go2:
        jal     solve_puzzle
        jal     check_dropoff
...
count_disjoint_regions:
		...
		li	 $t2, 0					#change angle to go right
		sw	 $t2, ANGLE
		li	 $t2, 1
		sw	 $t2, ANGLE_CONTROL
