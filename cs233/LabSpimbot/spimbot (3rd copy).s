...
get_astroids:
        ...
        # lw      $s0,    flag_puzzle
    	# beq	$s0,   0,    check_astroid		# if flag_puzzle is 1, go to solving puzzle
     	jal 	solve_puzzle
check_astroid:
...
go_to_station:
        # Make a branch whether to collect astroids or wait the station
        li	$t6,	281			# appoximate station x 281, 172
        li	$t7,	195			# appoximate station y 195, 54
################################################################################
...
y_smaller_loop_:
        ...
        jal	escape				# escape from the supernova
        lw	$t3,	BOT_Y			# SPIMbot.y
        bge	$t3,	195,	drop_or_wait	# SPIMbot.y = s.y
        j	y_smaller_loop_
...
y_greater_loop_:
        ...
        jal	escape				# escape from the supernova
        lw	$t3,	BOT_Y			# SPIMbot.y
        ble	$t3,	195,	drop_or_wait	# SPIMbot.y = a.y
        j	y_greater_loop_
################################################################################
...
drop_or_wait:
        ...
        lw	$s6,	STATION_LOC
        srl 	$s5,	$s6,	16		# s.x
        bne 	$t2,	281,	move_right	# b.x = t.x
        bne 	$s5,	281,	move_right	# s.x = t.x
        j 	drop

move_right:
        bge	$t2,	281,	move_left
        sw	$zero,	ANGLE			# DIRECTION: +x
        li	$t4,	1
        sw	$t4,	ANGLE_CONTROL		# CONTROL: abs
        j 	drop_or_wait
...
solve_puzzle:
	sub    $sp,    $sp,    16
        ...
        la	$a1,    puzzle_data	# canvas from puzzle struct
	add    $a0,    $a1,    16     # lines from puzzle struct
	la     $a2,    solution
	add    $t0,    $a2,    8
	sw     $t0,    4($a2)
...
count_disjoint_regions:
	...
	sw	 $t4, ANGLE_CONTROL
        ...
        sw	$s4, 0($s2)		# solution -> length = lines -> num_lines
        li      $s5, 0          # s5 = i
        lw      $s6, 4($s0)     # s6 = lines->coords[0]
        lw      $s7, 8($s0)     # s7 = lines->coords[1]
cdr_for_loop:
        bgeu    $s5, $s4, cdr_end_for
        mul     $t2, $s5, 4     # t2 = i*4
...
check_throw_puzzle:
	lw 	$t4, GET_ENERGY
        ble 	$t4, 900, submit_the_solution_done
        lw 	$t4, CHECK_OTHER_FROZEN
        beq	$t4, 1, submit_the_solution_done
        sw 	$zero, THROW_PUZZLE
submit_the_solution_done:
	sw	$t6, SUBMIT_SOLUTION
	sw	$zero, flag_puzzle

        la	$t4,    puzzle_data	# request puzzle
...
read_in_map:
        ...
        add	$s1,	$s7,	4		# $s1 = &astroids[50]
        mul	$s0,	$s0,	8
        add	$s0,	$s0,	$s1		# &astroids[i]
        lw	$t2,	0($s0)			# $t2 = x and y
        srl	$t4,	$t2,	16		# $s2 = a.x
...
station_enter_interrupt:
        ...
        li	$a1,	1
        sw	$a1,	flag_station		# set flag_station to 1
        j	interrupt_dispatch		# see if other interrupts are waiting
################################################################################
...
station_exit_interrput:
        ...
        li	$a1,	0
        sw	$a1,	flag_station		# set flag_station to 0
        j	interrupt_dispatch		# see if other interrupts are waiting
################################################################################
...
bot_freeze_interrupt:
        ...
        li	$a1,	1
        sw	$a1,	flag_freeze		# set flag_freeze to 1

        # la	$a1,    op_puzzle_data	# canvas from puzzle struct
        # add    $a0,    $a1,    16     # lines from puzzle struct
        # la     $a2,    solution
        # add    $a3,    $a2,    8
        # sw     $a3,    4($a2)
