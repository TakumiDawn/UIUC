...
################################################################################
go_to_station:
        li	$t6,	172			# appoximate station x 281, 172
        li	$t7,	54			# appoximate station y 195, 54
################################################################################
...
nvm:
        ...
        # sw     $a3,    4($a2)
        #
        # jal     count_disjoint_regions_freeze
        # la      $a3,    solution
        # add     $a3,    $a3,    4
        # sw      $zero,  -4($a3)
        # sw      $zero,  0($a3)
...
################################################################################
.globl count_disjoint_regions_freeze
count_disjoint_regions_freeze:
        sub     $sp, $sp, 64
        sw      $ra, 0($sp)
        sw      $s0, 4($sp)
        sw      $s1, 8($sp)
...
cdr_for_loop_freeze:
	...
	lw      $t6, 4($s2)     # t6 = solution->counts
        ...
        sw      $v0, 0($t7)     # solution->counts[i] = count
        addi    $s5, $s5, 1     # i++
        j       cdr_for_loop_freeze

cdr_end_for_freeze:

submit_the_solution_done_freeze:
	sw	$t6, UNFREEZE_BOT
	sw	$zero, flag_puzzle ###inccrement?
