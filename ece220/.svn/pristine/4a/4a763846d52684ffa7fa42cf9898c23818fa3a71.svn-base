...
//first helps to find the first entry, which has smallest row and col.
sp_tuples_node *first(sp_tuples *matrix, sp_tuples_node *current)
{
...
//ordermatrix helps to place the matrix, by th order of lower row & col.
void ordermatrix(sp_tuples *matrix)
{
  ...
  current=first(matrix,matrix->tuples_head);
  int tmpcol,tmprow;
  double tmpval;
...
//replace is a helper function that overwrite *check onto *current
void replace(sp_tuples *matrix,sp_tuples_node *current, sp_tuples_node *check)
{
...
void delzero(sp_tuples *matrix)
{
  ...
  {
    replace(matrix,matrix->tuples_head,matrix->tuples_head->next);
  }
...
void delduplicate(sp_tuples *matrix)
{
  ...
  while(current->next->next!=NULL) //Loop through the entire matrix to check duplicate entries for every entry.
  {
    ...
    while(check!=NULL)
    {
      ...
      {
      replace(matrix,current->next,check);
      check=current->next->next;
      }
      else
      {
 ...
 */
 void save_tuples(char * file_name, sp_tuples * mat_t)
 {
   ...
   fprintf(fp, "%d %d\n", mat_t->m, mat_t->n); //write the size
 ...
 /*
  * add_tuples adds two matrices of the same size
  */
 sp_tuples * add_tuples(sp_tuples * matA, sp_tuples * matB)
 {
  sp_tuples *matC;
  if(!(matA->n==matB->n&&matA->m==matB->m))
    return NULL; //Unable to add, return NULL.
 ...
 /*
  * mult_tuples does multiplication of two matrices
  * For each node in A, a nonzero element needs to be multiplied by every entry
  * in B where the row of B matches the column of A.
  */
