//Intro paragraph:
//This program will try to use tree to organize shapes in sush a way as the
//rectangles are organized in a bigger reactangle. This is very important to
//circuits designs and organizing things on silicon The trees will be sliced.
// Floorplanning is one major step in physical design. It assembles partitioned
// circuit modules into a rectangular chip to optimize a predefined cost metric
// such as area; it is particularly crucial because the resulting floorplan affects
// all the subsequent steps in physical design, such as cell placement,
// wire routing, and so on.
//In this MP, I will implement a floorplanner that layouts a set of rectangular
//modules and optimizes the packing area.
...
{
  if (ptr->module != NULL)
    return 1; //Return 1 if the given slicing tree node is a leaf node
  else
    return 0;
}
...
{
  if (ptr->module == NULL)
    return 1;// Return 1 if the given slicing tree node is an internal node
  else
    return 0;
}
...
int is_in_subtree(node_t* a, node_t* b)
{
  if ( (b->parent==a)||(a==b))
  {
    return 1; // Return 1 if the given subtree rooted at node 'b' resides in the subtree rooted at node 'a'.
  }
...
{
  //the height and the width of the modules are swapped.
  int temp;
  temp = ptr->module->w;
...
void recut(node_t* ptr)
{
  ...
  if (ptr->cutline == V)
  {
    ptr->cutline = H; // If the original cutline is a vertical cutline,change to H
  }
  else if (ptr->cutline == H)
  {
    ptr->cutline = V; // If the original cutline is a horizontal cutline,change to V
  }
  return;
}
...
void swap_module(node_t* a, node_t* b)
{
  ...
  // Swap the two modules between two given leaf nodes
  module_t* temp;
  temp = a->module;
  a->module = b->module;
  b->module = temp;
...
void swap_topology(node_t* a, node_t* b)
{
  ...
  //swap their parents' links to children
  if (a->parent->left==a)
    a->parent->left = b;
  else
    a->parent->right = b;
  ...
  if (b->parent->left==b)
    ...
    b->parent->right = a;
  //swap their links to their parents
    node_t* temp;
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression)
{
  if(ptr == NULL)
    return;
  // Traverse the left subtree by recursively calling the Postfix function.
